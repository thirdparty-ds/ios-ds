...
class DriverStation {
    ...
    convenience init(team: UInt32) {
        self.init(team: team, alliance: Alliance.Blue)
    }
    
    convenience init() {
        self.init(team: 0, alliance: Alliance.Blue)
    }
    ...
    func setGameMode(mode: GameMode) {
        DS_DriverStation_set_mode(ds, Mode(rawValue: mode.rawValue))
    }
    ...
    func isConnected() -> Bool {
        DS_DriverStation_trace(ds) & UInt8(TRACE_IS_ROBORIO) != 0
    }
    
    func isCodeAlive() -> Bool {
        DS_DriverStation_trace(ds) & UInt8(TRACE_ROBOT_CODE) != 0
    }
